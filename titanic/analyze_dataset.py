...
class DataSet(object):
    def __init__(self, train_file, test_file, x_index, y_index):
        self.train_file = train_file
        self.test_file = test_file
        self.x_index = x_index
        self.y_index = y_index
        self.print_line('train data', True)
        data_raw = self.read_csv_data(self.train_file)
        print(data_raw.info())
        print(data_raw.sample(10))
        self.print_line('test data', True)
        self.read_csv_data(self.test_file)
    ...
    def read_csv_data(self, file):
        df = pd.read_csv(file)
        self.print_describe(df)
        return df

    def print_describe(self, df):
        print(df.head())
        self.print_line('head()', True)
        print(df.tail())
        self.print_line('tail()', True)
        print(df.describe())
        self.print_line('describe()', True)
        print(df.isnull().sum())        
        self.print_line('isnull().sum()', True)
        
    def print_line(self, print_str, is_top):
        if is_top:
            print('********** ' + print_str + ' **********')
        else:
            print('********************')
...
if __name__ == "__main__":
    ...
    data = DataSet('data/test.csv', '', args.x_index, args.y_index)
    #data.read_data()
    data.read_csv_data()
